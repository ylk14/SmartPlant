...
export default function AdminUserDetailScreen({ route, navigation }) {
  ...
  const loadData = async () => {
    try {
      const freshUser = await fetchAdminUserById(
        (userData || FALLBACK_USER).user_id
      );
      const roleList = await fetchRoles();

      setCurrentUser(freshUser || userData || FALLBACK_USER);
      setRoles(roleList || []);
    } catch (err) {
  ...
  const isActive =
    currentUser.is_active === 1 || currentUser.is_active === true;
  ...
  const currentRole = roles.find(
    (r) => r.role_id === currentUser.role_id
  );
  const currentRoleName = currentRole ? currentRole.role_name : "Unknown role";

  const confirmRoleChange = (role) => {
    if (role.role_id === currentUser.role_id) {
    ...
    Alert.alert(
      "Assign Role",
      `Assign ${role.role_name} role to ${currentUser.username}?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Confirm",
          onPress: () => {
            saveChange({ role_id: role.role_id });
            setRoleMenuOpen(false);
          },
        },
      ],
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.infoCard}>
          ...
          />
          <DetailRow label="Role" value={currentRoleName} />
        </View>
        ...
        <View style={styles.controlsCard}>
          ...
          <View style={styles.controlRow}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={styles.roleSelectText}>{currentRoleName}</Text>
              <Ionicons
